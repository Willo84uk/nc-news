    ...
    .then(({ rows }) => {
      if (!rows.length) {
        return Promise.reject({
          status: 404,
          msg: "article not found with this article id",
        });
      }
      return { rows };
    });
...
exports.insertNewArticle = (author, title, body, topic, imgUrl) => {
  ...
  if (imgUrl) {
    (columnsToInsert += ", article_img_url"),
      (numberOfValuesToInsert += ", $5"),
      valuesToInsert.push(imgUrl);
  }
